...
@Component({
  selector: 'page-detail_giaodich_nh',
  templateUrl: 'detail_giaodich_nh.html'
})
export class DetailGiaoDichNHPage {
  ...
  constructor(private alertCtrl: AlertController,private toast: ToastController,public myServer:Server,public account:Account,public navCtrl: NavController,public navParams: NavParams) {
    ...
    this.myServer.postRequest("CTH/GetNH1",postData,(data)=>{
      var stt = data["stt"];
      if(stt == 1){
        data = data["data"];
        this.init(data);
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.myServer.getRequest("CTH/CancelNH",postData,(data)=>{
              var stt = data["stt"];
              if(stt == 1){
                this.init(data["data"]);
                this.presentToast(data["msg"]);
